...
//        }
        ...
        for(unsigned bCount = 0; bCount < ndatagrams; bCount++) {
            ...
            // Missing packets
                ...
                fprintf(stdout, "==================== Generated %u empty packets =====================\n", lostPackets);
            }
...
int main()
{
    ...
    if (pId == 0)  {
        ...
        initialiseReader(sampPerPacket, subsPerPacket, nPols, sampPerSecond, 
                         sampSize, "192.168.8.211", pipeId[1], port);
        ...
        DoubleBuffer doubleBuffer(65536 * 16, subsPerPacket, nPols);
        
        double a, b;
        float *packet = (float *) malloc(2048);
        while (1) {
            while (doubleBuffer.prepareRead(&a, &b) == NULL) {
                if (read(pipeId[0], packet, 2048) <= 0) {
