...
void SpeadBeamChunker::run()
{
    ...
    while(true)
    {
        ...
        {   
            ...
            unsigned offset = beamId * _samplesPerSubband * _subbandsPerHeap * sizeof(float) + payloadOffset;
            memcpy(_heap + offset, payload, payloadLen);

//            for (unsigned i = 0; i < payloadLen / 4; i++)
//                printf("%d - ", ((unsigned int *) payload)[i]);
//            printf("====\n====\n====\n====\n");
        ...
        if (_numPackets != _packetsPerHeap)
            std::cout << "Only read " << _numPackets << " from heap [" << _currHeapNumber << "]"  << std::endl;
        
        // Forward heap to Double Buffer
        //_buffer -> writeHeap(_numberOfBeams, _subbandsPerHeap, _samplesPerSubband, heap);
         _buffer -> writeHeap();
    }
}
