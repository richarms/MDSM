...
// Global arguments
unsigned samplesPerHeap = 128, port = 10000, samplesPerSecond = 78125, packetsPerHeap = 64, nbeams = 2;
...
int main(int argc, char *argv[])
{
    ...
    // TODO: Check if requested nsamp is a multiple of the incoming samples per heap

    // Initialise Circular Buffer and set thread priority
    DoubleBuffer doubleBuffer(nbeams, survey -> nchans, survey -> nsamp);
    doubleBuffer.start();
    doubleBuffer.setPriority(QThread::TimeCriticalPriority);
    
    // Initialise UDP Chunker and set thread priority
    SpeadBeamChunker chunker(port, nbeams, survey -> nchans, samplesPerHeap, samplesPerSecond, packetsPerHeap * nbeams);
    ...
    while(true) 
    {
        ...
        memcpy(inputBuffer, udpBuffer, survey -> nbeams * survey -> nchans * survey -> nsamp * sizeof(float));
